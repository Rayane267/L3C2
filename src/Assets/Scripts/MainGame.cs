...
public class MainGame : MonoBehaviour
{
    ...
    public GameObject CarteAvecEffets; // edited by Mengqian Xu L3C2
    ...
    public GameObject particleIceLance; // edited by Mengqian Xu L3C2
    public GameObject particlePoisonGas; // edited by Mengqian Xu L3C2
    public GameObject particleFireFiles; // edited by Mengqian Xu L3C2
    public GameObject particleElectricalSparks; // edited by Mengqian Xu L3C2
    public GameObject particleDustExplosion; // edited by Mengqian Xu L3C2
    public GameObject  ParticleFireBall;// edited by Mengqian Xu L3C2
    ...
    public List<int> damageValueTeam1 = new List<int>(6);
    public List<int> damageValueTeam2 = new List<int>(6);

    public GameObject UIStatistic;
    ...
    void Start()
    {
        ...
        else
        {
            ...
            else if (startGameData.mapChosen == 2) // edited by Mengqian Xu L3C2
            {
                hexaGrid.createGridFromFile(Application.streamingAssetsPath + "/Data/Map/CarteAvecEffets");
                tileW = hexaGrid.w; tileH = hexaGrid.h;
                CarteAvecEffets.SetActive(true);
                foreach (Hexa hexa in hexaGrid.hexaList)
                {
                    if (hexa.type != HexaType.GROUND && hexa.type != HexaType.BONUS)
                    {
            ...
            else if (startGameData.mapChosen == 1)// edited by Mengqian Xu L3C2
            {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (i < startGameData.charsTeam1.Count) hexaGrid.addChar(startGameData.charsTeam1[i], tileW / 2 - 4 + 2 + i, tileH - 2, 0, i);
                if (i < startGameData.charsTeam2.Count) hexaGrid.addChar(startGameData.charsTeam2[i], tileW / 2 - 4 + 2 + i, 2, 1, i);
            }
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {   // Back to menu
                if (mousePos.x >= Screen.width / 2 - 150 && mousePos.x < Screen.width / 2 + 150 && 
                    mousePos.y >= Screen.height / 2 - 90 && mousePos.y < Screen.height / 2 - 90 + 60)
                {
                    Debug.Log("BINGO");
                    // EVALUATE AI HARD
                    // to delete
                ...
                if (mousePos.x >= Screen.width / 2 - 150 && mousePos.x < Screen.width / 2 + 150 &&
                    mousePos.y >= Screen.height / 2 - 90 -61 && mousePos.y < Screen.height / 2 - 90 + 1)
                {
                    UIStatistic.SetActive(true);
                    String text = "Les dégats totaux de l'équipe 1 : " + damageValueTeam1[0] + "\n";
                    for (int i = 0; i < MainGame.startGameData.charsTeam1.Count; i++)
                    {
                        text += startGameData.charsTeam1[i].ToString() + " (" + (i + 1) + ")" + " : " + damageValueTeam1[i+1] +
                                " (" +((double)damageValueTeam1[i+1] / damageValueTeam1[0]).ToString("0.0%")+ ")" +"\n";
                    }
                    
                    String text2 = "Les dégats totaux de l'équipe 2 : " + damageValueTeam2[0] + "\n";
                    for (int i = 0; i < MainGame.startGameData.charsTeam2.Count; i++)
                    {
                        text2 += startGameData.charsTeam2[i].ToString() + " (" + (i + 1) + ")" + " : " + damageValueTeam2[i+1] +
                                " (" +((double)damageValueTeam2[i+1] / damageValueTeam2[0]).ToString("0.0%")+ ")" +"\n";
                    }

                    statistic.transform.GetChild(0).GetComponent<Text>().text = text2;
                    statistic.transform.GetChild(2).GetComponent<Text>().text = text;
                }
            }
        }
    ...
    //get the hexa of the zone bonus
    public  Hexa getBonus(){
        return caseBonus;
    }
    ...
    //Author : ?? a MOdifier
    // edited by L3C2 Rayane TALEB, Mengqian Xu
    void actionUseAttack(ActionType attack, Hexa hexaDestination)
    {
        ...
        if (hexaDestination != null && hexaGrid.hexaInSight(currentCharControlled.x, currentCharControlled.y, hexaDestination.x, hexaDestination.y, attackUsed_.range))
        {
            ...
            //Particles for Druide
            else if (currentCharControlled.charClass == CharClass.DRUIDE) // edited by Rayane TALEB ,Mengqian xu L3C2
            {
                this.animationDruide(hexaDestination);
            }
            //Particles for archer
            else if (currentCharControlled.charClass == CharClass.ARCHER) // edited by Mengqian Xu L3C2
            {
                this.animationArcher(hexaDestination);
            }
            //Particles for envouteur
             else if (currentCharControlled.charClass == CharClass.ENVOUTEUR) // edited by Mengqian Xu L3C2
            {
                this.animationEnvouteur(hexaDestination);
            }
            //Particles for voleur
              else if (currentCharControlled.charClass == CharClass.VOLEUR) // edited by Mengqian Xu L3C2
            {
                this.animationVoleur(hexaDestination);
            }

            //Particles for valkyrie
             else if (currentCharControlled.charClass == CharClass.VALKYRIE) // edited by Mengqian Xu L3C2
            {
                this.animationValkyrie(hexaDestination);
            }
            //Particles for geurrier
             else if (currentCharControlled.charClass == CharClass.GUERRIER) // edited by Mengqian Xu L3C2
            {
                this.animationGeurrier(hexaDestination);
            }

            if (attackUsed_.attackEffect == CharsDB.AttackEffect.DAMAGE || attackUsed_.attackEffect == CharsDB.AttackEffect.POISON)
            {
                int damage = attackUsed_.effectValue;
                if (attackUsed_.attackEffect == CharsDB.AttackEffect.POISON)
                    damage = 3;
                //find the index of current char controlled
                if (currentCharControlled.team == 0)
                {
                    int index = currentCharControlled.id;
                    damageValueTeam1[0] += damage;
                    damageValueTeam1[index+1] += damage;
                }
                else if(currentCharControlled.team == 1)
                {
                    int index = currentCharControlled.id;
                    damageValueTeam2[0] += damage;
                    damageValueTeam2[index+1] += damage;
                }
            }
    ...
    //Particles for Druide
    // edited by Mengqian Xu L3C2
    void animationDruide(Hexa hexaDestination){
        GameObject go = GameObject.Instantiate(ParticleFireBall);
        go.transform.position = Hexa.hexaPosToReal(hexaDestination.x, hexaDestination.y, 0);
        go.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(go,5);
    ...
    //Particles for Geurrier
    // edited by Mengqian Xu L3C2
    void animationGeurrier(Hexa hexaDestination){
        
        GameObject go = GameObject.Instantiate(particleDustExplosion);
        go.transform.position = Hexa.hexaPosToReal(hexaDestination.x, hexaDestination.y, 0);
        go.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(go, 5);
    ...
    //Particles for Archer
    // edited by Mengqian Xu L3C2
    void animationArcher(Hexa hexaDestination){
        GameObject go = GameObject.Instantiate(particleIceLance);
        go.transform.position = Hexa.hexaPosToReal(hexaDestination.x, hexaDestination.y, 0);
        go.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(go, 5);
    }
    ...
    //Particles for envouteur
    // edited by Mengqian Xu L3C2
    void animationEnvouteur(Hexa hexaDestination){
        GameObject go = GameObject.Instantiate(particlePoisonGas);
        go.transform.position = Hexa.hexaPosToReal(hexaDestination.x, hexaDestination.y, 0);
        go.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(go, 5);
    }
    ...
    //Particles for thief
    // edited by Mengqian Xu L3C2
    void animationVoleur(Hexa hexaDestination){
        GameObject go = GameObject.Instantiate(particleFireFiles);
        go.transform.position = Hexa.hexaPosToReal(hexaDestination.x, hexaDestination.y, 0);
        go.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(go, 5);
    ...
    //Particles for Valkyrie
    // edited by Mengqian Xu L3C2
    void animationValkyrie(Hexa hexaDestination){
        GameObject go = GameObject.Instantiate(particleElectricalSparks);
        go.transform.position = Hexa.hexaPosToReal(hexaDestination.x, hexaDestination.y, 0);
        go.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(go, 5);
    ...
    void nextTurn()
    {
        ...
        if (currentCharControlled.totalDamage2 >= 22 && currentCharControlled.charClass == CharClass.GUERRIER)
        {
            currentCharControlled.totalDamage2 -= 0;
            currentCharControlled.skill2Available = true;
        }
        else if (currentCharControlled.totalDamage2 >= 16)
        {
            currentCharControlled.totalDamage2 -= 16;
            currentCharControlled.skill2Available = true;
        }
    ...
    public void loadGame()
    {
        ...
        //If the map is "Carte Avec Effets" which has the effect speciaux
        // edited by Mengqian Xu L3C2
        ...
        for (int i = 0; i < nbChar && reader.Read(); i++)
        {
            ...
            Character c = new Character(cCharClass, cX, cY, cTeam, i);
    ...
    void useAttackDamageCase(Character c)
    {
        ...
        if (currentCharControlled.dmgbuff == true)
        {
            ...
            if (currentCharControlled.charClass == CharClass.VOLEUR && actionType == ActionType.SKILL1) //edited by rayane
            {
                ...
                dmgValueDisp1.GetComponent<DamageValueDisplay>().setValue(c.x, c.y - 1, "+ 2 HP", Color.green, 55);
                currentCharControlled.HP += 2;
            }
                ...
                case CharClass.VOLEUR:
                    {
                        ...
                        UIAction.transform.GetChild(6).gameObject.SetActive(true);
                        UIAction.transform.GetChild(5).transform.GetChild(0).GetComponent<Text>().text = "ATK : " + skillHelp.effectValue;
                        UIAction.transform.GetChild(6).transform.GetChild(0).GetComponent<Text>().text = "Le voleur se guérit de 2 HP";
                        break;
                    }
                case CharClass.ARCHER:
                    {
    ...
    void displayActionButtons()
    {
        ...
        if (currentCharControlled.skill2Available == false)
        {
            if (currentCharControlled.charClass == CharClass.GUERRIER)
            {
                UIAction.transform.GetChild(3).transform.GetChild(0).GetComponent<Text>().text = currentCharControlled.totalDamage2 + " / 22";// edited by L3C2 Rayane TALEB
            }
            else
            {
                UIAction.transform.GetChild(3).transform.GetChild(0).GetComponent<Text>().text = currentCharControlled.totalDamage2 + " / 16";// edited by L3C2 Rayane TALEB
            }
        }
        else
        {
