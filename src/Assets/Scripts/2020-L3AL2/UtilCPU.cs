    ...
    public class UtilCPU
    {
        ...
        */
        ...
        public static bool setBestEnvouteurSkillVsTarget(Character envouteur, Character target, List<Character> team)
        {
         ...
         /// <summary>
        /// Function used by maxDamageToTarget
        /// Calculates what would be the best Hexa to use the buff skill 2 on
        /// to maximise damage on a target, and buffes that unit. 
        /// Doesn't create an action sequence as this is only used for calculations of the enemy team's movements.
        /// </summary>
        /// <param name="envouteur">The sorcerer buffing</param>
        ...
        /// <param name="team">The envouteur's team</param>
        /// <returns>True if a unit can be buffed, false else</returns>
        public static bool setBestEnvouteurSkillVsTarget2(Character envouteur, Character target, List<Character> team)
        {
            if (envouteur.getPA() > 0 && envouteur.isSkillUp())
            {
                int unitExtraPADamage;
                ...
                if (maxDamage[0] != 0)
                {
                    ...
                    hex = takeAOEIntoAccount(envouteur, target, envouteur.getClassData().skill_2,
                            new Pair<List<Character>, List<int>>(maxDamageDealer, maxDamage), ActionType.SKILL2);
                    charList = AIUtil.hexaGrid.getCharWithinRange(hex.x, hex.y, envouteur.getClassData().skill_2.rangeAoE);
        ...
        public static bool gotEnvouteurInTeam(int team)
        {
         ...
         /// <summary>
        /// Checks if <code>target</code> got a Mage in his team
        /// </summary>
        /// <param name="c">The character</param>
        /// <param name="pos">The character's coordinates</param>
        /// <param name="target">The target</param>
        ...
        public static bool gotMageInTeam(int team)
        {
            foreach (Character charac in AIUtil.hexaGrid.charList)
                if (team == charac.team)
                    if (charac.charClass == CharClass.MAGE)
                        return true;
            return false;
        }

         /// <summary>
        /// Checks if <code>target</code> got a thief in his team
        /// </summary>
        /// <param name="c">The character</param>
        /// <param name="pos">The character's coordinates</param>
        /// <param name="target">The target</param>
        /// <returns>True if <code>target</code> is in range and false otherwise</returns>
        ///Edited by Mengqian Xu L3C2
        public static bool gotVoleurInTeam(int team)
        {
            foreach (Character charac in AIUtil.hexaGrid.charList)
                if (team == charac.team)
                    if (charac.charClass == CharClass.VOLEUR)
                        return true;
            return false;
        }

         /// <summary>
        /// Checks if <code>target</code> got a Druide in his team
        /// </summary>
        /// <param name="c">The character</param>
        /// <param name="pos">The character's coordinates</param>
        /// <param name="target">The target</param>
        /// <returns>True if <code>target</code> is in range and false otherwise</returns>
        ///Edited by Mengqian Xu L3C2
        public static bool gotDruideInTeam(int team)
        {
            foreach (Character charac in AIUtil.hexaGrid.charList)
                if (team == charac.team)
                    if (charac.charClass == CharClass.DRUIDE)
                        return true;
            return false;
        }

        /// <summary>
        /// Checks if <code>target</code> got a Valkyrie in his team
        /// </summary>
        /// <param name="c">The character</param>
        /// <param name="pos">The character's coordinates</param>
        /// <param name="target">The target</param>
        /// <returns>True if <code>target</code> is in range and false otherwise</returns>
        ///Edited by Mengqian Xu L3C2
        public static bool gotValkyrieInTeam(int team)
        {
            foreach (Character charac in AIUtil.hexaGrid.charList)
                if (team == charac.team)
                    if (charac.charClass == CharClass.VALKYRIE)
                        return true;
            return false;
        }

         /// <summary>
        /// Checks if <code>target</code> got a geurrier in his team
        /// </summary>
        /// <param name="c">The character</param>
        /// <param name="pos">The character's coordinates</param>
        /// <param name="target">The target</param>
        /// <returns>True if <code>target</code> is in range and false otherwise</returns>
        ///Edited by Mengqian Xu L3C2
        public static bool gotGeurrierInTeam(int team)
        {
            foreach (Character charac in AIUtil.hexaGrid.charList)
                if (team == charac.team)
                    if (charac.charClass == CharClass.GUERRIER)
                        return true;
            return false;
        }

     /// <summary>
        /// Checks if <code>target</code> got a archer in his team
        /// </summary>
        /// <param name="c">The character</param>
        /// <param name="pos">The character's coordinates</param>
        /// <param name="target">The target</param>
        /// <returns>True if <code>target</code> is in range and false otherwise</returns>
        ///Edited by Mengqian Xu L3C2
        public static bool gotArcherInTeam(int team)
        {
            foreach (Character charac in AIUtil.hexaGrid.charList)
                if (team == charac.team)
                    if (charac.charClass == CharClass.ARCHER)
                        return true;
            return false;
        }
        ...
        /// <summary>
        /// Return the character closest to the point
        /// </summary>
        ///<param name = currentChar> The current character ID </param>
        ///Edited by Mengqian Xu L3C2
        public static Character closestToPoint(int ourteam, HexaGrid hexas, Point p){
            List<Character> allyTeam = getCharacterTeamList(ourteam, hexas);
            Character close = null;
            int min = 100;
            int dis;
            foreach (Character ally in allyTeam){
                dis = AIUtil.hexaGrid.getWalkingDistance(ally.x, ally.y, p.x, p.y);
                if(min > dis ){
                    min = dis;
                    close = ally;
                }
        ...
        /// <summary>
        /// Find the point of the "zone bonus" and return  the character  closest to the "zone bonus"
        /// </summary>
        ///<param name = currentChar> The current character ID </param>
        ///Edited by Mengqian Xu L3C2
        public static Character closestToZoneBonus(int ourteam,HexaGrid hexas){
            MainGame mg = new MainGame();
            int x = mg.getBonus().getx();
            int y = mg.getBonus().gety();
            Point destination = new Point(x,y);
            Character c = closestToPoint(ourteam,hexas,destination);
            return c;        
        }

        /// <summary>
        /// return true if one of the character in the team is in the range of the zone bonus
        /// </summary>
        ///<param name = currentChar> The current character ID </param>
        ///Edited by Mengqian Xu L3C2
        public static bool checkIfInZoneBonus(int ourteam,HexaGrid hexas){
            List<Character> allyTeam = getCharacterTeamList(ourteam, hexas);
            foreach(Character c in allyTeam){
                if(AIUtil.hexaGrid.getHexa(c.x, c.y).type == HexaType.BONUS)
                {
                    return true;
                }
            }
         ...
         /// <summary>
        /// Return the best point for a character to go, if he want to get closer to the zone bonus
        /// </summary>
        //<param name = currentChar> The current character ID </param>
        ///<param name = c >The character </param>
        ///<param name = bonus >The case of zone bonus </param>
        /// <returns>The best point</returns>
        ///Edited by Mengqian Xu L3C2
        public static Point bestPointToGoToZoneBonus(int currentTeam, HexaGrid hexas, Character c, Hexa bonus, int r)
        {
            Debug.Log("CharacterFollowWithoutRisk");
            Dictionary<Point, int> valueList = new Dictionary<Point, int>();
            Point Pos = new Point(c.x, c.y);
            Hexa goTo = null;
            int min = 999;

            foreach (Point p in pointsAroundHexa(Pos, r ))
            {
                goTo = hexas.getHexa(p.x, p.y);
                if (goTo != null && goTo.type == HexaType.GROUND && goTo.charOn == null)
                    valueList.Add(p, AIUtil.hexaGrid.getWalkingDistance(p.x, p.y, bonus.getx(), bonus.gety()));
            }
            ...
            Point closest = null;
            foreach (Point p in pointsAroundHexa(Pos, r))
                if (AIUtil.hexaGrid.getWalkingDistance(p.x, p.y, bonus.getx(), bonus.gety()) < min)
                {
                    min = AIUtil.hexaGrid.getWalkingDistance(p.x, p.y, bonus.getx(), bonus.gety());
                    closest = p;
                }
        ...
        /// <summary>
        /// Returns the List<Character> ranked by their HP and within attack range of the healer , from lowest to highest
        /// </summary>
        /// <param name="c">The character</param>
        /// <returns>The character's class as a string</returns>
        ...
        {
            List<Point> spellRange = AIUtil.hexaGrid.findHexasInSight2(envouteur.x, envouteur.y, 3);
     ...
     /// <summary>
        /// Return the best Point for a Theif to use his spell
        /// </summary>
        /// <param name="toSort">The list to sort</param>
        /// <param name="charsInOrder">The sorting order</param>
        /// <returns>The sorted list</returns>
        public static Point bestPointForAOEBoostVoleur(int currentTeam, HexaGrid hexas, Character voleur)
        {
            List<Point> spellRange = AIUtil.hexaGrid.findHexasInSight2(voleur.x, voleur.y, 3);
        ...
        /// <summary>
        /// Return true if the soigneur can move to he's ally without puting himself at risk
        /// </summary>
        /// <param name="toSort">The list to sort</param>
        /// <param name="charsInOrder">The sorting order</param>
        /// <returns>The sorted list</returns>
        ...
        ///Edited by Mengqian Xu L3C2
        /// <summary>
        /// Return the best point for the healer to go, if he want to get closer to an ally who is the most seriously injured closest to him
        /// </summary>
        /// <param name="toSort">The list to sort</param>
        /// <param name="charsInOrder">The sorting order</param>
        /// <returns>The sorted list</returns>
        
        public static Point healerFollowMostImportant(int currentTeam, HexaGrid hexas, Character soigneur,List<CharacterCPU> allyRankedByMissingHP)
        {
        
            Debug.Log("healerFollowWithoutRisk");
            Dictionary<Point, int> valueList = new Dictionary<Point, int>();
            Point healerPos = new Point(soigneur.x, soigneur.y);
            Hexa goTo = null;
            int min = 999;
            
            CharacterCPU choose = choosingAlly(allyRankedByMissingHP, currentTeam, hexas, soigneur);
            ...
            foreach (Point p in pointsAroundHexa(healerPos, 3))
            {
                goTo = hexas.getHexa(p.x, p.y);
                if (goTo != null && goTo.type == HexaType.GROUND && goTo.charOn == null)
                    valueList.Add(p, AIUtil.hexaGrid.getWalkingDistanceCPU(p.x, p.y, choose.x, choose.y));
            }
            ...
            Point closest = null;
            foreach (Point p in pointsAroundHexa(healerPos, 3))
                if (AIUtil.hexaGrid.getWalkingDistanceCPU(p.x, p.y, choose.x, choose.y) < min)
                {
                    min = AIUtil.hexaGrid.getWalkingDistanceCPU(p.x, p.y, choose.x, choose.y);
                    closest = p;
                }
            foreach (KeyValuePair<Point, int> position in valueList)
            {
        ...
        /// <summary>
        /// TODO: Incomplete
        /// Determine which skills the character can release
        /// </summary>
        /// <param name="character">The character to check</param>
        /// <returns>If the character's skill 1 can be used, return 1, if skill 2 can be used, return 2, if both can be used, return 3</returns>
        /// Edited by  Mengqian Xu L3C2
        public static int skill1OrSkill2IsAvailable(Character character)
        {
            int x = 0;
            if (character.isSkillUp())
            {
                x = 1;
            }
            if (character.isSkill2Up())
            {
                x = 2;
            }
            if (character.isSkillUp() && character.isSkill2Up())
            {
                x = 3;
            }
            return x;
        ...
        /// <summary>
        /// The maximum damage a warrior can achieve if using the second skill
        /// </summary>
        /// <param name="character">The character to check</param>
        /// <returns>return the maximu damge</returns>
        /// Edited by  Mengqian Xu L3C2
        public static int skill2WarriorMaxDamge(Character target, Character c, List<Character> group)
        {
            int max = 0;
            Point tempPos;
            tempPos = new Point(c.getX(), c.getY());
            if (c.getCharClass() == CharClass.GUERRIER)
            {
                max = calcUnitExtraPADamage(c, target);
            }
        ...
        /// <summary>
        /// The maximum damage a warrior can achieve for each turn
        /// </summary>
        /// <param name="character">The character to check</param>
        /// <returns>return the maximu damge</returns>
        /// Edited by  Mengqian Xu L3C2
        public static int skillWarriorMaxDamge(Character target, Character c, List<Character> group)
        {
            Point tempPos;
            tempPos = new Point(c.getX(), c.getY());
            int max = 0;
            if (c.getCharClass() == CharClass.GUERRIER)
            {
                if (skill1OrSkill2IsAvailable(c) == 3)
                {
                    if (skill2WarriorMaxDamge(target, c, group) > 5)
                    {
                        max = skill2WarriorMaxDamge(target, c, group);
                    }
                    else
                    {
                        max = 5;
                ...
                else if (skill1OrSkill2IsAvailable(c) == 2)
                {
                    max = skill2WarriorMaxDamge(target, c, group);
                }
                else
