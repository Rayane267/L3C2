...
public class AIUtil{
			...
			if (c.team == currentChar.team){
				if (hexaGrid.hexaInSight(currentChar.x,currentChar.y,c.x,c.y,currentChar.getClassData().basicAttack.range)){
					...
					switch (c.charClass){
						...
						case CharClass.DRUIDE    : value = 7; break;
						case CharClass.VALKYRIE    : value = 8; break;
	...
	///<summary>
	///Checks if there is a Druide on the field 
	///</summary>
	///<param name = currentChar> The current character controlled by the AI </param>
	///<returns> Returns true or false </returns>
	///Edited by Mengqian Xu L3C2
	public static bool isThereADruide(int myCharID)
	{
		Character currentChar = hexaGrid.charList[myCharID];
		int nbDruideChar = 0;
		foreach (Character c in hexaGrid.charList)
	...
	///<summary>
	///Checks if there is a Valkyrie on the field 
	///</summary>
	///<param name = currentChar> The current character controlled by the AI </param>
	///<returns> Returns true or false </returns>
	///Edited by MengqianXu L3C2
	public static bool isThereAValkyrie(int myCharID)
	{
		Character currentChar = hexaGrid.charList[myCharID];
		int nbValkyrieChar = 0;
		foreach (Character c in hexaGrid.charList)
		{
			if ((c.team != currentChar.team))
			{
	...
	///<summary>
	///Checks if there is a Thief on the field 
	///</summary>
	///<param name = currentChar> The current character controlled by the AI </param>
	///<returns> Returns true or false </returns>
	///Edited by MengqianXu L3C2
	public static bool isThereAVThief(int myCharID)
	{
		Character currentChar = hexaGrid.charList[myCharID];
		int nbThiefChar = 0;
		foreach (Character c in hexaGrid.charList)
	...
	///<summary>
	/// Determines if the current character can use their skill1 on their target
	///</summary>
	///<param name = c> Current Character </param>
	///<param name = target> Target </param>
	///<returns> True or false </returns>
	...
	///<summary>
	/// Determines if the current character can use their skill2 on their target
	///</summary>
	///<param name = c> Current Character </param>
	///<param name = target> Target </param>
	///<returns> True or false </returns>
	///Edited by MengqianXu L3C2
	public static bool canTargetSkill2(Character c,Character target){ 
		if (c.skillAvailable){
			switch (c.charClass){
				case CharClass.GUERRIER :
				case CharClass.ARCHER : 
				case CharClass.MAGE :
					return target.team != c.team && canTarget(c.x,c.y,target.x,target.y,c.getClassData().skill_2.range,c.getClassData().skill_2.rangeAoE);
				case CharClass.VOLEUR :
					return target.team != c.team && /*target.charClass != CharClass.GUERRIER &&*/ canTarget(c.x,c.y,target.x,target.y,c.getClassData().skill_2.range,c.getClassData().skill_2.rangeAoE);
				case CharClass.SOIGNEUR :
					return target.team == c.team && target != c && target.HP < target.HPmax && canTarget(c.x,c.y,target.x,target.y,c.getClassData().skill_2.range,c.getClassData().skill_2.rangeAoE);	
				case CharClass.ENVOUTEUR :
					return target.team == c.team && target != c && target.PA <= target.getClassData().basePA && canTarget(c.x,c.y,target.x,target.y,c.getClassData().skill_2.range,c.getClassData().skill_2.rangeAoE);
				case CharClass.DRUIDE:
					return target.team == c.team && target != c && target.PA <= target.getClassData().basePA && canTarget(c.x,c.y,target.x,target.y,c.getClassData().skill_2.range,c.getClassData().skill_2.rangeAoE);
				case CharClass.VALKYRIE :
					return target.team == c.team && target != c && target.PA <= target.getClassData().basePA && canTarget(c.x,c.y,target.x,target.y,c.getClassData().skill_2.range,c.getClassData().skill_2.rangeAoE);
	...
	///<returns> True or false </returns>
	///Edited by MengqianXu L3C2
	public static bool canTargetWithMovementSkill2(Character c,Character target){
		if (c.skill2Available){
			int cX = c.x; int cY = c.y;
			for (int i=1;i<c.PA;i++){
				List<Point> listH = hexaGrid.findAllPaths(c.x,c.y,c.PM*i);
				foreach (Point charPos in listH){
					c.updatePos2(charPos.x,charPos.y,hexaGrid);
					if (canTargetSkill2(c,target)){
						c.updatePos2(cX,cY,hexaGrid);
						return true;
					}
				}
		...
		foreach (Character c in hexaGrid.charList){
			...
			if (c.team != toCheckChar.team && c.charClass != CharClass.SOIGNEUR && c.charClass != CharClass.ENVOUTEUR){
				...
				// 0 PM
				{
					...
					//bool skill2 = canTargetSkill2(c,toCheckChar);
	...
	///<returns> Returns a list of hexas </returns>
	///Edited by Mengqian Xu L3C2
	public static List<Point> getHexasWhereCharIsInRangeSkill2(List<Point> hexas,Character c,Character target){
		List<Point> hexas2 = new List<Point>();
		if (hexas.Count > 0){
			int cX = c.x; int cY = c.y;
			foreach (Point p in hexas){
				c.updatePos2(p.x,p.y,hexaGrid);
				if (canTargetSkill2(c,target)) hexas2.Add(new Point(p.x,p.y));
			}
			c.updatePos2(cX,cY,hexaGrid);
		}
		return hexas2;
	...
	///<summary>
	/// Get list of characters in attack range for the skill 1
	///</summary>
	///<param name = c> </param>
	///<returns>  </returns>
	...
	///<summary>
	/// Get list of characters in attack range for the skill 2
	///</summary>
	///<param name = c> </param>
	///<returns>  </returns>
	///Edited by Mengqian Xu L3C2
	public static List<Character> getTargetableCharsInRangeSkill2(Character c){
		List<Character> r = new List<Character>();
		foreach (Character c2 in hexaGrid.charList) if (canTargetSkill2(c,c2)) r.Add(c2);
		return r;
	}
	...
	///<returns>  </returns>
	///Edited by Mengqian Xu L3C2
	public static Character getMainTargetSkill2(Character c){
		//Character c = hexaGrid.charList[charID];
		List<Character> l = getTargetableCharsInRangeSkill2(c);
		if (l.Count > 0){
			List<int> threatTmp = new List<int>();
			threatTmp.Add(threatAtHexa(l[0].x,l[0].y,c,l[0]));
			int minThreat = threatTmp[0];
			...
			foreach (Character c2 in hexaGrid.charList){
				switch (c.charClass){
					...
					case CharClass.ARCHER :
					case CharClass.MAGE :
						if (c.team != c2.team && canTargetWithMovementSkill2(c,c2)) l.Add(c2); break;
					
					case CharClass.SOIGNEUR :
					case CharClass.ENVOUTEUR:
						if (c.team == c2.team && c != c2 && canTargetWithMovementSkill2(c,c2)) l.Add(c2); break;
					case CharClass.DRUIDE:
					case CharClass.VALKYRIE :
					break;
				}
	...
	public class AIHard2 {
		...
		///<param name = currentChar > </param>
		///<returns>  </returns>
		public static Point doSkill2Pos(Character currentChar,Character target){
			//Character currentChar = hexaGrid.charList[currentChar];
			//Character target = hexaGrid.charList[targetID];
			List<Point> hexas1  = hexaGrid.findAllPaths(currentChar.x,currentChar.y,currentChar.PM*(currentChar.PA-1));
			List<Point> hexas2 = getHexasWhereCharIsInRangeSkill2(hexas1,currentChar,target);
			List<Point> hexas3;
			List<Point> hexas4;
			if (hexas2.Count == 0){
				hexas1 = hexaGrid.findAllPaths(currentChar.x,currentChar.y,currentChar.PM*(currentChar.PA));
		...
		public static List<ActionAIPos> doSkill(Character currentChar){
			...
			if (canTargetSkill2(currentChar,target)){
				Point targetHexa = getPosToUseSkill(currentChar,target);
				if (sequence.Count < currentChar.PA) sequence.Add(new ActionAIPos(MainGame.ActionType.SKILL2,new Point(targetHexa.x,targetHexa.y)));
			}
